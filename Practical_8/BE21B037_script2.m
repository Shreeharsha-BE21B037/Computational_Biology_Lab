...
%%%%%% uncommment the below section to run with euler simple integration
%%%%%% %%%%%%%%%%%
% function main()
%     % Initial conditions vector
%     x0 = zeros(13,1);
...
%     t_end = 100;
%     dt = 0.01;  % Time step
%     tspan = t_start:dt:t_end;  % Time points array
% 
...
%         t = tspan(i-1);         % Current time
%         xdot = f(t, x(i-1,:)'); % Compute the derivative (dx/dt)
%         x(i,:) = x(i-1,:) + dt * xdot';  % Euler step: x_new = x_old + dt * dx/dt
%     end
% 
%     % Plot the results
%     plot(tspan, x);
...
%     legend('aca', 'oaa', 'coa', 'cit', 'icit', 'akg', 'ssa', 'suc', 'sca', 'fa', 'mal', 'gly', 'mth');
% end
